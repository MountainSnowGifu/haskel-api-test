...
-- | GET /task ハンドラ
--
--   AuthProtect "token-auth" により Servant が Username を解決して渡す。
--   型の流れ:
--     AuthHandler Request Username → Username
--       → getTaskHandler pool user
--       → Eff '[TaskRepo, IOE] Task
--       → (runTaskRepo) → Eff '[IOE] Task
--       → (runEff)      → IO Task
--       → (liftIO)      → Handler Task
getTaskHandler :: MSSQLPool -> Username -> Handler Task
getTaskHandler pool _user =
  liftIO $ runEff (runTaskRepo pool getTaskResult)

getTaskAllHandler :: MSSQLPool -> Username -> Handler [Task]
getTaskAllHandler pool _user =
  liftIO $ runEff (runTaskRepo pool getTaskAllResult)

-- | POST /task ハンドラ
postTaskHandler :: MSSQLPool -> Username -> Task -> Handler Task
