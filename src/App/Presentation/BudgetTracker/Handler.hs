...
getRecordsAllHandler :: (AuthPrincipal -> RecordRunner) -> AuthPrincipal -> Handler [RecordResponse]
getRecordsAllHandler mkRun user = do
  records <- liftIO $ mkRun user fetchAllRecords
  return (map toRecordResponse records)

postRecordHandler :: (AuthPrincipal -> RecordRunner) -> AuthPrincipal -> PostRecordRequest -> Handler RecordResponse
...
deleteRecordHandler :: (AuthPrincipal -> RecordRunner) -> AuthPrincipal -> Int -> Handler DeleteRecordResponse
...
getSummaryHandler :: (AuthPrincipal -> RecordRunner) -> AuthPrincipal -> Maybe Text -> Handler SummaryResponse
